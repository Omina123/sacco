...
#             if profile.employment_status == 'CONTRACT':
#                 if not profile.contract_expiry:
#                     messages.error(request, "Contract expiry date missing.")
#                     return redirect('member_dashboard')
...
#                     messages.error(
#                         request,
#                         f"Loan must end before {max_allowed_date.strftime('%B %Y')}."
#                     )
#                     return redirect('member_dashboard')
...
#                 if duration > 48:
#                     messages.error(request, "Normal loans max 48 months.")
#                     return redirect('apply_loan')
#             else:
#                 if duration > 12:
#                     messages.error(request, "This loan type max 12 months.")
#                     return redirect('apply_loan')
...
#             for g in guarantors:
#                 count = Guarantor.objects.filter(
#                     member=g.member,
#                     loan__status__in=['pending_guarantors', 'pending', 'approved', 'disbursed']
#                 ).count()
...
#                     messages.error(
#                         request,
#                         f"{g.member.user.get_full_name()} already guarantees 3 loans."
#                     )
#                     return redirect('apply_loan')
...
#                             raise ValueError("Only members can guarantee loans.")
...
#             # --- NEW: 🚨 CONTRACT EXPIRY CHECK ---
#             if profile.employment_status == 'CONTRACT':
#                 if not profile.contract_expiry:
#                     messages.error(request, "Contract expiry date missing. Please update your profile.")
#                     return redirect('member_dashboard')
                
#                 # Calculate the max allowed date (Expiry Date - 2 Months)
#                 max_allowed_date = profile.contract_expiry - relativedelta(months=2)
#                 # Calculate when this loan would end
#                 loan_end_date = date.today() + relativedelta(months=duration)

#                 if loan_end_date > max_allowed_date:
#                     readable_expiry = max_allowed_date.strftime('%B %Y')
#                     messages.error(request, f"Loan duration too long. For contract terms, loans must be cleared by {readable_expiry} (2 months before expiry).")
#                     return redirect('member_dashboard')

#             # --- 2. DURATION VALIDATION (Normal/Xmas) ---
#             if loan.loan_type == 'normal':
#                 if duration > 48:
#                     messages.error(request, "Normal Loans cannot exceed 48 months.")
#                     return redirect('member_dashboard')
...
#             # --- 3. INTEREST & AFFORDABILITY (Keep your existing math here) ---
#             # ... (Calculation for monthly_installment, take_home_limit, etc.) ...

#             # --- 4. 🚨 GUARANTOR LIMIT CHECK (MAX 3 LOANS) ---
#             guarantors_to_save = formset.save(commit=False)
#             for g in guarantors_to_save:
#                 # Count how many active loans this person is currently guaranteeing
#                 guarantee_count = Guarantor.objects.filter(
#                     member=g.member,
#                     loan__status__in=['pending_guarantors', 'pending', 'approved', 'disbursed']
#                 ).count()
...
#                 if guarantee_count >= 3:
#                     messages.error(request, f"Guarantor {g.member.user.get_full_name()} has reached the limit of 3 active guarantees.")
#                     return redirect('member_dashboard')
...
#                     for g in guarantors_to_save:
#                         if g.member.user.user_type != '4':
#                             raise ValueError(f"User {g.member.user.get_full_name()} is not a regular Member.")
#                         g.loan = loan
#                         g.save()
